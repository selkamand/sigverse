...
style="width:100%" />
```
The **sigverse** does **NOT** seek to provide custom implementations for running signature analyses.

It simply provides tools for evaluating, exploring, and reporting the results signature analyses done with any existing signature analysis frameworks (although we recommend [sigminer](https://github.com/ShixiangWang/sigminer) if you're using R).
...
**Signatures**: representing the profile of a mutational signature. data.frames  with 3 columns (channel, type, and fraction)

**Signature Collections**: Lists of signature data.frames, where name of list entry is the name of the signature

**Signature Aetiology Collections**: Metadata about a signature. data.frames with 2 columns (signature, aetiology)
...
models.
data.frames with 4 columns (channel, type, fraction, count)
