core <- c("sigshared", "sigstash", "sigstats", "sigvis", "sigstory",
          "sigsim")
...
sigverse_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }

  header <- cli::rule(
    left = cli::style_bold("Attaching core sigverse packages"),
    right = paste0("sigverse ", package_version_h("sigverse"))
  )
